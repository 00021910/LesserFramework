          ...
          // Gets all attributes as object or array
            if(object.as.toLowerCase() == "object") {
              ...
              return console.error(
                "Invalid way to get attributes. Please, use method in right way"
                );
...
export function html(literals, ...vars) {
  ...
  //if(statement == null || eval(statement) == true){
    if (!template) throw new SyntaxError('Use template functions to render view.') 
  ...
  export function $ReadFile(htmlFilePath) {
     eval(`import rawData from ${htmlFilePath}`)
     return rawData
  }
